...
function loancomparison_get_stored_settings($table) {
	...
	if (!$settings['sortby']) {
		...
		$settings['sort_fees']		= $settings['sort_term'] = false;
	}
	...
	if (!$settings['columnsort']) {
		...
		$settings['sortby_infolink'] = $settings['sortby_loancost']	= $settings['sortby_otherinfo']	= $settings['sortby_creditscore'] = $settings['sortby_interestamount'] = $settings['sortby_bankname'] = false;
	};
...
function loancomparison_get_default_interest ($table) {
	...
	update_option(LC_INTEREST.$table,$default);
}
