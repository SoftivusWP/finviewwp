...
// Sets up the tabs on the settings page
function loancomparison_admin_tabs($current = 'settingsone') { 
	
	$allowed_html = loancomparison_callback_allowed_html();
...
function loancomparison_tabbed_page() {
	...
	echo wp_kses('<p style="color:red"><strong>READ THIS!</strong> There will be no further updates to this plugin except to fix security bugs and ensure compatibility with the latest version of WordPress. The plugin isn\'t going to be deleted but I have built a new Comparison Plugin that is so much easier to manage and update. <a href="https://comparisonplugin.com/">Click here to see the demo</a>.</p>',$allowed_html);
...
function loancomparison_settings() {
	...
	$warning = $nodata = $content = $drop = $grid = $before = $after = $none = $non = $float = $always = false;
	...
	if (isset($_POST['update-bank-list']) && check_admin_referer("save_loancomparison")) {
		...
		if (!(int) $settings['pageid']) {
			...
			loancomparison_admin_notice("Some settings not saved because a bank page was not created");
		} else {
			loancomparison_admin_notice("Lender page list settings saved");
		}
	...
	if( isset( $_POST['Insert']) && check_admin_referer("save_loancomparison")) {
		...
		update_option( LC_INTEREST.$theform, $interest );
		loancomparison_admin_notice("Lender pages created.");
	}
	...
	if( isset( $_POST['Default']) && check_admin_referer("save_loancomparison")) {
		
		loancomparison_get_default_interest($theform);
		loancomparison_admin_notice("Sample dataset imported.");
	}
	...
	$principal ='<table class="step-table"><tr>';
	foreach ($settings['loan'] as $k => $v) {
		if (!$v['step']) continue;
		$s = ceil((((int)$v['max'] - (int)$v['min']) / (int)$v['step']) + 1);
		for ($i = 0; $i < $s; $i++) {
			if ($v['min'] < $minloan) $minloan = $v['min'];
			if ($v['max'] > $maxloan) $maxloan = $v['max'];
			$principal .='<td>'.$step.'<br><em>';
			if ($i == 0) $principal .= ($v['min'] * 1);
			else $principal .= ((int)$v['step'] * $i) + (int)$v['min'];
			$principal .='</em></td>';
			$step++;
			$cells++;
			if ($cells == 20) {$principal .= '</tr><tr>';$cells=0;}
		}
	...
	foreach ($settings['period'] as $k => $v) {
		...
		for ($i = 0; $i < $s; $i++) {
			...
			$step++;
			$cells++;
			if ($cells == 35) {$terms .= '</tr><tr>';$cells=0;}
		}
	}
	$terms .='</tr></table>';
	...
	$query = new WP_Query( 's=[loancomparison table='.$theform.']' );
	...
	<fieldset style="border: 1px solid #c3c4c7;padding:10px;margin-bottom:10px;">
	<h2>'.__('General Settings', 'loan-comparison').'</h2>';
	
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		$content .='<p>'.__('Add the comparison table to your site using the shortcode', 'loan-comparison').': [loancomparison table='.$theform.']</p>';
		if (isset($thistitle)) $content .= '<p>Calculator '.$theform.' is currently used on the <a href="'.$permalink.'"><strong>'.$thistitle.'</strong></a> '.$post_type.'.</p>';
	} else {
		$content .='<p>'.__('Add the comparison table to your site using the shortcode', 'loan-comparison').': [loancomparison]</p>';
	}
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		$content .='<h2>'.__('Form Selection', 'loan-comparison').'</h2>
		<p class="description">'.__('Select the form you want to manage and click on the Change Form button', 'loan-comparison').'. '.__('If the form has been added to a page you will see the page title in the selector and a link above to the page', 'loan-comparison').'.</p>
		<p><select name="calculator">';
		for ($i= 1; $i<=25; $i++) {
			$title = false;
			$query = new WP_Query( 's=[loancomparison table='.$i.']' );
			if ($query->have_posts()){
				while ( $query->have_posts() ) {
					$query->the_post();
					$title = ' ('.get_the_title().')';
			...
			$content .='<option value="'.$i.'"'.$selected.'>Table '.$i.$title.'</option>';
		}
	...
	<h2>'.__('Lender Data', 'loan-comparison').'</h2>';
	
	if ($nodata) $content .='<p class="description">'.__('For the plugin to work you need a set of data', 'loan-comparison').'. '.__('Click on the button to add lender data or import a sample dataset so you can play with the settings', 'loan-comparison').'</p>';
	
	$content .='<p class="description">'.__('The only data you really need for each lender is the min and max valsues for loan amount and term and an interest rate', 'loan-comparison').'. '.__('The table may look a little odd without a logo but it will function', 'loan-comparison').'.</p>
	
	<table id="sorting">
	<tbody id="loancomparison_sort">';
	
	$bankminloan = 100000000;
	$bankmaxloan = 0;
	
	foreach ($interest as $k => $v) {
		if ($bankminloan > $v['min_loan']) {
			$bankminloan = $v['min_loan'];
		}
		if ($bankmaxloan < $v['max_loan']) {
			$bankmaxloan = $v['max_loan'];
		}
	...
	<p><input type="button" name="AddBank" id="loancomparisonAddBank" onclick="loancomparison_add_bank()" class="button-secondary" value="+ Add New Lender" /> ';
	if ($nodata) $content .= '<input type="submit" name="Default" class="button-primary" style="color: #FFF;" value="Import Sample Dataset" />';
	else $content .= '<input type="submit" name="Submit" class="button-primary" style="color: #FFF;" value="Save Table '.$theform.' Settings" />';
	$content .= ' <input type="submit" name="Reset" class="button-primary" style="color: #FFF;" value="Reset" onclick="return window.confirm( \'Are you sure you want to reset the lender data?\' );"/></p>
	...
	$content .= '<fieldset style="border: 1px solid #c3c4c7;padding:10px;margin-bottom:10px;">
		...
		<p class="description">'.__('The form heading displays above the sliders', 'loan-comparison').'. '.__('The features display below the form heading', 'loan-comparison').'. '.__('Both are centred on the page', 'loan-comparison').'</p>
	...
	<p class="description">'.__('The sliders display next to each other above the table of results', 'loan-comparison').'.</p>
	<p class="description">'.__('The table below lets you set a loan amount and term with unequal steps', 'loan-comparison').'. '.__('For example 100 to 999 in steps of 100 then 1000 to 10,000 in steps of 1000', 'loan-comparison').'. '.__('Or just a simple range with equal steps', 'loan-comparison').'.</p>
	<p class="description">'.__('The term slider settings have a period marker so the plugin knows if the steps are in days, months or years', 'loan-comparison').'.</p>';
	...
	if ($minloan < $bankminloan || $maxloan > $bankmaxloan) $warning = true;
	if ($warning) $content .= '<p><span style="color:red;font-weight:900;">'.__('Warning!', 'loan-comparison').'</span> ';
	if ($minloan < $bankminloan) $content .= __('The minimum loan amount ', 'loan-comparison').'('.$settings['currency'].$minloan.') '.__('is less than the loweest loan a lender offers', 'loan-comparison').' ('.$settings['currency'].$bankminloan.'). ';
	if ($maxloan > $bankmaxloan) $content .= __('The maximum loan amount ', 'loan-comparison').'('.$settings['currency'].$maxloan.') '.__('is greater than the highest loan a lender offers', 'loan-comparison').' ('.$settings['currency'].$bankmaxloan.'). ';
	if ($warning) $content .= __('This may mean there are no results to display', 'loan-comparison');
	
	$content .= '<p class="description">'.__('The step number of each slider position are given in the table below', 'loan-comparison').'. '.__('Use these step numbers to set the initial postions', 'loan-comparison').'. '.__('These step numbers are also used in the bank data below to set the min term and max term', 'loan-comparison').'.</p>';
	...
	$content .= '<p><b>'.__('Amount Slider', 'loan-comparison').':</b> '.__('Label', 'loan-comparison').': <input type="text" style="width:7em" name="loanslider" value ="' . $settings['loanslider'] . '" />&nbsp;&nbsp;&nbsp;'.__('Currency Symbol', 'loan-comparison').': <input type="text" style="width:3em;" name="currency" value ="' . $settings['currency'] . '" />&nbsp;&nbsp;&nbsp;'.__('Initial Position', 'loan-comparison').': <input type="text" style="width:3em;" name="loaninitial" value ="' . $settings['loaninitial'] . '" /></p>';
	
	$content .= '<p><b>'.__('Term Slider', 'loan-comparison').':</b> '.__('Label', 'loan-comparison').': <input type="text" style="width:7em" name="termslider" value ="' . $settings['termslider'] . '" />&nbsp;&nbsp;&nbsp;'.__('Period Labels', 'loan-comparison').'&nbsp;&nbsp;&nbsp;<em>Singular:</em> <input type="text" style="width:5em;" name="daylabel" value ="' . $settings['daylabel'] . '" />&nbsp;<input type="text" style="width:5em;" name="monthlabel" value ="' . $settings['monthlabel'] . '" />&nbsp;<input type="text" style="width:5em;" name="yearlabel" value ="' . $settings['yearlabel'] . '" />&nbsp;&nbsp;&nbsp;<em>Plural:</em> <input type="text" style="width:5em;" name="dayslabel" value ="' . $settings['dayslabel'] . '" />&nbsp;<input type="text" style="width:5em;" name="monthslabel" value ="' . $settings['monthslabel'] . '" />&nbsp;<input type="text" style="width:5em;" name="yearslabel" value ="' . $settings['yearslabel'] . '" />&nbsp;&nbsp;&nbsp;'.__('Initial Position', 'loan-comparison').': <input type="text" style="width:3em;" name="periodinitial" value ="' . $settings['periodinitial'] . '" /></p>';
	
	$content .= '<p><strong>'.__('Hide', 'loan-comparison').'</strong> <input type="checkbox" name="loanhide"  value="checked" ' . $settings['loanhide'] . '/>'.__('Loan slider', 'loan-comparison').'&nbsp;&nbsp;&nbsp;<input type="checkbox" name="termhide"  value="checked" ' . $settings['termhide'] . '/>'.__('Term slider', 'loan-comparison').'&nbsp;&nbsp;&nbsp;<input type="checkbox" name="nobuttons"  value="checked" ' . $settings['nobuttons'] . '/>'.__('Buttons', 'loan-comparison').' ('.__('labels and values move to the ends of the slider', 'loan-comparison').')</p>';
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		<p class="description">'.__('The features for each bank or lender can be filtered', 'loan-comparison').'. '.__('The filters display below the siders as a set of checkboxes or a dropdown ', 'loan-comparison').'.</p>
		...
		<h2>'.__('Lender Filters', 'loan-comparison').'</h2>
		<p class="description">'.__('This option lets you filter by lender name').'. '.__('The filters display below the siders as a set of checkboxes or a dropdown', 'loan-comparison').'.</p>
		...
		<p class="description">'.__('Sort the results by the selected options').'. '.__('You can order and select which of the outputs appear in the sorting dropdown', 'loan-comparison').'.</p>
		...
		<tr>
		<td>'.__('Lender Name Label', 'loan-comparison').': <input type="text" name="sortbankname" value ="' . $settings['sortbankname'] . '" /></td>
		...
		<p class="description">'.__('The credit scores are a set of buttons that display below the sliders and filters the results according to the selected credit rating.').'.</p>
		...
		<p class-="description">'.__('Show the number of matches abouve the results table', 'loan-comparison').'.</p>
		...
		<p class-="description">'.__('Show the column labels above the results table', 'loan-comparison').'.</p>
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		<p class="description">'.__('The table of results has a logo on the left and a button on the right', 'loan-comparison').'. '.__('In between you can have a number of optional outputs').'. '.__('You can select up to 6 columns but 4 fit nicely on the page', 'loan-comparison').'.</p>
		<p class="description">'.__('Drag and drop the elements below to change the display order', 'loan-comparison').'.</p>';
		
		$sort = explode(",", $settings['columnsort']);
		$sortby = 'sortby_';
		$content .='<ul class="sorting" id="loancomparison_column_rsort">';
	...
	$content .='<h2>'.__('Column Labels', 'loan-comparison').'</h2>
	...
	<td>'.__('Repayment Period', 'loan-comparison').': <input type="text" name="termslider" value ="' . $settings['termslider'] . '" /></td>
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
	   ...
	   <td>'.__('Lender Name', 'loan-comparison').': <input type="text" name="banknamelabel" value ="' . $settings['banknamelabel'] . '" /></td>
	...
	$content .='</table>
	<h2>'.__('Output Columns Options', 'loan-comparison').'</h2>
	<p><input type="checkbox" name="addfees" value="checked" '.$settings['addfees'].'> '.__('Add fees to principal', 'loan-comparison').' <input type="radio" name="whenfees" value="beforeinterest" ' . $beforeinterest . ' />Before calculations&nbsp;&nbsp;&nbsp;
	...
	<p class="description">'.__('Ratings are set in the bank data and display below the logo as a series of stars', 'loan-comparison').'.</p>
	
	<p><input type="checkbox" name="showrating" value="checked" '.$settings['showrating'].'> '.__('Show ratings', 'loan-comparison').'.</p>';
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		<p class="description">'.__('The promo ribbons display across the top left corner of the bank details', 'loan-comparison').'.</p>
		...
		<p class="description">'.__('These aren\'t most elegant option and can really slow down your site but give a visual comparison indicator', 'loan-comparison').'.</p>
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		<h2>'.__('Show Lender Name', 'loan-comparison').'</h2>
		<p class="description">'.__('This displays the bank name (alt text) above the logo and the ribbon label if required', 'loan-comparison').'.</p>
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		<p class="description">'.__('Display a numbered icon in the top left corner', 'loan-comparison').'.</p>
	...
	<p class="description">'.__('The apply now buttom displays at the right of each banmk row', 'loan-comparison').'. '.__('Sponsored listings display an appropriate message', 'loan-comparison').'. '.__('You can select to hide the buttons', 'loan-comparison').'.</p>
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		<h2>'.__('Highlights', 'loan-comparison').'</h2>
		<p class="description">'.__('The highlighs for each lender display with a checkmark below the output columns on each listing', 'loan-comparison').'.</p>
		<p><input type="checkbox" name="showchecks" value="checked" '.$settings['showchecks'].'> '.__('Show highlights below columns', 'loan-comparison').'</p>
	
		<table>
		
			<tr>
				<td>'.__('Highlight', 'loan-comparison').' 1: <input type="text" name="check1" value ="' . $settings['check1'] . '" /></td>
				<td>'.__('Highlight', 'loan-comparison').' 2: <input type="text" name="check2" value ="' . $settings['check2'] . '" /></td>
				<td>'.__('Highlight', 'loan-comparison').' 3: <input type="text" name="check3" value ="' . $settings['check3'] . '" /></td>
			</tr>
			<tr>
				<td>'.__('Highlight', 'loan-comparison').' 4: <input type="text" name="check4" value ="' . $settings['check4'] . '" /></td>
				<td>'.__('Highlight', 'loan-comparison').' 5: <input type="text" name="check5" value ="' . $settings['check5'] . '" /></td>
				<td>'.__('Highlight', 'loan-comparison').' 6: <input type="text" name="check6" value ="' . $settings['check6'] . '" /></td>
			</tr>
	...
	<p class="description">'.__('Usually a legal requirment, the example text can be display in each listing or in the more info drop down', 'loan-comparison').'.</p>
	...
	<p class="description">'.__('The more info section is hidden until you click the more info button', 'loan-comparison').'. '.__('There are four sections you can label and populate as required', 'loan-comparison').'.</p>
	...
	<p class="description">'.__('The review link takes the visitor to a new page where further information about the bank can be displayed', 'loan-comparison').'.</p>
	...
	$content .= '<fieldset style="border: 1px solid #c3c4c7;padding:10px;margin-bottom:10px;">
		...
		<p class="description">'.__('This setting controls the number of results to display on page load', 'loan-comparison').'. '.__('Setting the vlaue to a high number means you display all the listings', 'loan-comparison').'. '.__('A low number means you have to click to see all the results', 'loan-comparison').'.</p>
	...
	<p class="description">'.__('These settings control how the links work on the logo and button.').'. '.__('You also have the option to add a URL query to the link.').'.</p>
	<table>
	...
	<h2>'.__('Number Formatting', 'loan-comparison').'</h2>
	<p class="description">'.__('These settings let you control how the numberical outputs display.').'.</p>
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		...
		$content .= '<fieldset style="border: 1px solid #c3c4c7;padding:10px;margin-bottom:10px;">
			...
			<h2>'.__('Lender List Location', 'loan-comparison').'</h2>
			<p class="description">'.__('There may be occasions where you want to show the sliders on one page and link to a new page to display the results').'.</p>
			...
			<p>'.__('Lender list URL', 'loan-comparison', 'loan-comparison').'</p>
	...
	// Bank Page Creation
		...
		<h2>'.__('Lender Page Creation', 'loan-comparison').'</h2>
		<p class="description">'.__('You have the option here to generate a set of bank pages.').'. '.__('The bank page will display the listing for that bank according to the settings selected below.').'.</p>
...
function loancomparison_styles() {
	...
	<tr>
	<td colspan="3"><h3>'.__('Slider labels', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td width="30%">'.__('Label Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="slider-label-colour" value="' . $style['slider-label-colour'] . '" /></td>
	
	<td width="30%">'.__('Output Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="slider-output-colour" value="' . $style['slider-output-colour'] . '" /></td>
	
	<td width="30%">'.__('Buttons Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="buttoncolour" value="' . $style['buttoncolour'] . '" /></td>
	</tr>
	
	<tr>
	<td colspan="3"><h3>'.__('Slider Bar', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td>'.__('Thickness', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="slider-thickness" value="' . $style['slider-thickness'] . '" />&nbsp;px</td>
	
	<td>'.__('Normal Background', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="slider-background" value="' . $style['slider-background'] . '" /></td>
	
	<td>'.__('Revealed Background', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="slider-revealed" value="' . $style['slider-revealed'] . '" /></td>
	</tr>
	
	<tr>
	<td colspan="3"><h3>'.__('Slider Handle', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td>'.__('Background', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="handle-background" value="' . $style['handle-background'] . '" /></td>
	
	<td>'.__('Border colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="handle-border" value="' . $style['handle-border'] . '" /></td>
	
	<td>'.__('Thickness', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="handle-thickness" value="' . $style['handle-thickness'] . '" />&nbsp;px</td>
	</tr>
	
	<tr>
	<td>'.__('Corners', 'loan-comparison').'<br>
	<input type="text" style="width:2em" name="handle-corners" value="' . $style['handle-corners'] . '" />&nbsp;%</td>
	
	<td>'.__('Square Slider', 'loan-comparison').'<br>
	<input type="checkbox" name="slider-block" ' . $style['slider-block'] . ' value="checked" /> (this option removes the round ends of the slider and makes the handle the same height as the slider)</td>
	...
	<p><input type="checkbox" name="filterlabelhide" value="checked" '.$style['filterlabelhide'].'> '.__('Hide label on small screens', 'loan-comparison').'</p>
	<p>Breakpoint: <input type="text" style="width:4em" name="filterlabelbreakpoint" value="' . $style['filterlabelbreakpoint'] . '" />px</p>
	
	</fieldset>
	
	<fieldset style="border: 1px solid #c3c4c7;padding:10px;margin-top:10px;">

	<h2>'.__('Listing Styles', 'loan-comparison').'</h2>
	<table width="100%">
	
	<tr>
	<td colspan="3"><h3>'.__('Lender Box', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td width="30%">'.__('Background', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="bank-bankground" value="' . $style['bank-bankground'] . '" /></td>
	
	<td width="30%">'.__('Border Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="bank-border-colour" value="' . $style['bank-border-colour'] . '" /></td>
	...
	<tr>
	<td>'.__('Border thickness', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="bank-border-thickness" value="' . $style['bank-border-thickness'] . '" /> px</td>
	
	<td>'.__('Padding', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="bank-padding" value="' . $style['bank-padding'] . '" /> px</td>
	
	<td>'.__('Bottom Margin', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="bank-bottom-margin" value="' . $style['bank-bottom-margin'] . '" /> px</td>
	</tr>
	
	<tr>
	<td>'.__('Alternate backgrounds', 'loan-comparison').'<br>
	<input type="checkbox" name="bank-alternate" value="checked" '.$style['bank-alternate'].'> '.__('Alternate background colour', 'loan-comparison').' ('.__('normal background is white', 'loan-comparison').').</td>
	
	<td>'.__('Alternate background colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="bank-alternate-background" value="' . $style['bank-alternate-background'] . '" /></td>
	...
	<tr>
	<td colspan="3"><h3>'.__('Outputs', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td>'.__('Label Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="bank-label-colour" value="' . $style['bank-label-colour'] . '" /></td>
	
	<td>'.__('Ouput Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="bank-output-colour" value="' . $style['bank-output-colour'] . '" /></td>
	...
	<tr>
	<td colspan="3"><h3>'.__('Apply Now Button', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td>'.__('Label Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="button-label-colour" value="' . $style['button-label-colour'] . '" /></td>
	
	<td>'.__('Background', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="button-background-colour" value="' . $style['button-background-colour'] . '" /></td>
	
	<td>'.__('Background Hover', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="button-background-hover" value="' . $style['button-background-hover'] . '" /></td>
	</tr>
	
	<tr>
	<td>'.__('Missing Link', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="button-nolink" value="' . $style['button-nolink'] . '" /></td>
	
	<td>'.__('Border Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="button-border-colour" value="' . $style['button-border-colour'] . '" /></td>
	...
	<tr>
	<td>'.__('Border Thickness', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="button-border-thickness" value="' . $style['button-border-thickness'] . '" /> px</td>
	
	<td>'.__('Border Radius', 'loan-comparison').'<br>
	<input type="text" style="width:3em" name="button-border-radius" value="' . $style['button-border-radius'] . '" /> px</td>
	...
	<tr>
	<td colspan="3"><h3>'.__('More Info', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td>'.__('Label Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="more-link" value="' . $style['more-link'] . '" /></td>

	<td>'.__('Info Headers', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="more-headers" value="' . $style['more-headers'] . '" /></td>
	
	<td>'.__('Info Content', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="more-colour" value="' . $style['more-colour'] . '" /></td>
	</tr>
	
	<tr>
	<td colspan="3"><h3>'.__('Ribbons', 'loan-comparison').'</h3></td>
	</tr>
	...
	<tr>
	<td colspan="3"><h3>'.__('Row Numbers', 'loan-comparison').'</h3></td>
	</tr>
	
	<tr>
	<td>'.__('Number Colour', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="numbering-color" value="' . $style['numbering-color'] . '" /></td>

	<td>'.__('Background', 'loan-comparison').'<br>
	<input type="text" class="loancomparison-color" name="numbering-background" value="' . $style['numbering-background'] . '" /></td>
...
function loancomparison_upgrade () {
	...
	if( isset( $_POST['Upgrade']) && check_admin_referer("save_loancomparison")) {
		...
		$form = '<div class="loancomparison-options"><h2 style="clear:both;">'.__('Waiting for PayPal...', 'loan-comparison').'</h2>
	...
	if (!($loancomparisonkey['authorised'] ?? '')) {
		...
		<p>'.__('All for $53. Which I think is a bit of a bargain', 'loan-comparison').'.</p>
...
function loancomparison_scripts_init() {
	...
	wp_enqueue_script('loancomparison-media', plugins_url('settings.js', __FILE__ ), array( 'wp-color-picker' ), false, true );
}
...
function build_bank_row($settings, $k, $v) {
	...
	<input type="checkbox" name="interest['.$k.'][blocklink]" value="checked" '.( isset($v['blocklink'])? $v['blocklink'] : "" ).'> '.__('Hide link', 'loan-comparison').'<br>'
	.__('Terms & Conditions Link', 'loan-comparison').':<input type="text" name="interest['.$k.'][termlink]" value="'.( isset($v['termlink'])? $v['termlink'] : "" ).'"><br>'
	.__('Terms & Conditions Text', 'loan-comparison').':<input type="text" name="interest['.$k.'][terms_condition]" value="'.( isset($v['terms_condition'])? $v['terms_condition'] : "" ).'">'
	.__('Card Name', 'loan-comparison').':<input type="text" name="interest['.$k.'][popularcard]" value="'.( isset($v['popularcard'])? $v['popularcard'] : "" ).'">'
	.__('Reviews', 'loan-comparison').':<input type="text" name="interest['.$k.'][count_reviews]" value="'.( isset($v['count_reviews'])? $v['count_reviews'] : "" ).'">';
	...
	if (isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
		$content .= '<p><input type="checkbox" name="'.$k.'" value="checked"> '.__('Create Lender Page', 'loan-comparison').'<p></p>
		<input type="checkbox" name="interest['.$k.'][bankfilter]" value="checked" '.$v['bankfilter'].'> '.__('Add to Lender Filters', 'loan-comparison').'</p>';
	}
	...
	<td style="width:3%"><a class="remove_this">[X]</a></td>
	</tr>';
