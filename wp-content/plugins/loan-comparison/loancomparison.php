...
function loancomparison_display($atts)
{
	...
	$outputA['authorized'] = isset($loancomparisonkey['authorised']) ? $loancomparisonkey['authorised'] : false;
	...
	// Title and Features
	if ($settings['formtitle'] && isset($loancomparisonkey['authorised']) && $loancomparisonkey['authorised']) {
	...
	if (!$settings['replacebutton'] || $atts['ratespage']) {
		...
		// Table of Results
		$output .= '<div class="loancomparison_rates custom_loancomparison"></div>
...
.loancomparison-numbering {color: ' . $style['numbering-color'] . ';background: ' . $style['numbering-background'] . ';}';
	...
	if ($style['filterlabelhide']) $data .= '@media only screen and (max-width: ' . $style['filterlabelbreakpoint'] . 'px) {.loancomparison-filterlabel li.label { display: none; }}';
...
function loancomparison_callback_allowed_html()
{
