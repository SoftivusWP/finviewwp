jQuery(document).ready(function($) {
	...
	function loancomparisonCalculate(e) {
		...
		for (i in banks) {
			...
			// rajan
			if (rates.alternate) row = '<div class="card_area"><div class="bank_box'+((rc % 2)? ' bank_offset':'')+'" style="'+extrastyle+'">';
			else row = '<div class="card_area">  <div class="bank_box" style="'+extrastyle+'">';
			if (bank.popularcard) row += '<div class="interest_badge">'+bank.popularcard+'</div>';
			...
			row += '<div class="logo_rating"><div class="colmd8 bank_logo"><a href="'+bank.logolink+'"'+sponsored+logonofollow+logoblank+'><img src="'+bank.logo+'" alt="'+bank.alt+'" /></a></div>';
			...
			row += '<div class="colmd8 showrating">';
			if (sc_rates.showrating && !sc_rates.sortby_rating) row += loancomparison_repeat(bank.rating,rates.maxrating);
			row += '</div>';
			if (bank.count_reviews) row += '<div class="count_reviews">'+bank.count_reviews+'</div>';
			row += '</div>';
			...
			if (!rates.authorized) {
				...
				else row += '<div class="bank_details cus_repay"><span>'+sc_rates.repaymentlabel+'</span><b class="headingThree mx-2">'+rates.cb+loancomparison_doubledigit(total.repayment,rates).loancomparison_separator(rates)+rates.ca+'</b>/ Month';
				// if (bank.example) row += '<div class="colmd6">'+bank.example+'</div>';
				if (rates.showexample && rates.examplelocation == 'drop') row += '<div class="colmd6 showexample">'+bank.example+'</div>';
				row += '</div>';
				
				row += '<div class="bank_details_area"> <div class="bank_details"><span>'+sc_rates.interestlabel+'</span><b>'+bank.mininterest+'%';
				if (bank.maxinterest) row += ' - '+bank.maxinterest+'%';
				row += '</b></div>';
	
				row += '<div class="bank_details"><span>'+sc_rates.termslider+'</span><b>'+bank.min_term;
				if (bank.max_term) row += ' - '+bank.max_term+' Years';
				row += '</b></div>';
				
				row += '<div class="bank_details"><span>'+sc_rates.totallabel+'</span><b>'+rates.cb+loancomparison_doubledigit(total.total,rates).loancomparison_rounding(rates)+rates.ca+'</b></div></div>';
			...
			if (!sc_rates.singlebank) {
				row += '<div class="colmd4 cus_btn_area">';
				if(bank.terms_condition) row += '<div class="terms_condition"><a href="'+bank.termlink+'">'+bank.terms_condition+'</a></div>';
				row += '<div class="bank_apply">';
				...
				var review = '<a class="review" href="'+bank.logolink+'"'+reviewtarget+'>'+rates.reviewlabel+'</a>';
	
				if (review) row += '<div class="bank_inside">'+review+'</div>';
			...
			// 	row += '<div class="colmd6"><div class="bank_inside">'+drop+'</div>';
			// 	row += '</div>';
			// 	row += '</div>';
