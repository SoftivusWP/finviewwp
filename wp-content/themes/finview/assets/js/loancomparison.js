...
function loancomparisonCalculate(e) {
	...
	// rajan
		...
		if (!rates.authorized) {
			...
			else row += '<div class="bank_details"><span>'+sc_rates.repaymentlabel+'</span><b>'+rates.cb+loancomparison_doubledigit(total.repayment,rates).loancomparison_separator(rates)+rates.ca+'</b>/Month';
			...
			row += '<div class="bank_details"><span>'+sc_rates.termlabel+'</span><br><b>'+bank.min_term;
			if (bank.max_term) row += ' - '+bank.max_term;
			row += '</b></div>';
